<!-- Pemanggil Javascript untuk menu upload data pelatihan -->
<?php include('proccess_pelatihan.php'); ?>
...
<ul class="nav nav-tabs">
    <li class="active"><a id="pointer" href="#home" data-toggle="tab"><b>Form Upload Data Pelatihan</b></a></li>
</ul>
<br>
...
<form method="post">
    <table>
        <tr>
            ...
            <? if(empty($norm)){?>
            <a id="pointer" onclick="transformasi()" class="btn btn-primary normal"><i class="glyphicon glyphicon-refresh"></i> Transformasi Data Pelatihan</a><? } ?></td>
        </tr>
    </table>
</form>
<hr>
<!-- Menampilkan data peltihan asli dalam bentuk tabel -->
<table id="pelatihan" class="display" cellspacing="0" width="100%">
    <thead>
        <tr>
...
<script>
    $(document).ready(function() {
        ...
        $('#pelatihan tfoot th').each(function() {
            var title = $(this).text();
            var inp = '<input type="text" class="form-control" placeholder="' + title + '" />';
            $(this).html(inp);
        });

        //Fungsi untuk memanggil datatables di controller localhost/JST/tabel/datatables_pelatihan
        //dan ditampilkan ke dalam tabel dengan id user_view
        var table = $('#pelatihan').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "<?php echo base_url('tabel/datatables_pelatihan'); ?>",
                "type": "POST"
            }
        });
...
<div id="norm">Data Pelatihan Yang Telah Di Transformasi </div>

<!-- Menampilkan data pelatihan yang sudah ditransformasikan ke dalam nilai range [0,1] bentuk tabel -->
<table id="transformasi" class="display" cellspacing="0" width="100%">
    <thead>
        <tr>
            <th id="th1">ID Lokasi</th>
            <th>Lokasi</th>
...
<script>
    $(document).ready(function() {
        //Fungsi untuk kolom pencarian pada footer table
        $('#transformasi tfoot th').each(function() {
            var title = $(this).text();
            var inp = '<input type="text" class="form-control" placeholder="' + title + '" />';
            $(this).html(inp);
        });

        //Fungsi untuk memanggil datatables di controller localhost/JST/tabel/datatables_transformasi_pelatihan
        //dan ditampilkan ke dalam tabel dengan id transformasi
        var table = $('#transformasi').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "<?php echo base_url('tabel/datatables_transformasi_pelatihan'); ?>",
                "type": "POST"
            }
        });
...
<!-- CSS menu upload data pelatihan -->
<style type="text/css">
	td{
		padding: 10px 20px;
	}
